...
const RepayLoan = () => {
  ...
  const generateProof = () => {
    const hash = Web3.utils.keccak256(loanId + proofHash);
    setProofHash(hash);
  };
  ...
  const handleRepayLoan = async () => {
    ...
    try {
      await contract.methods.repayLoanWithProof(loanId, proofHash).send({ from: accounts[0] });
      setStatus('Loan Repayment Successful');
      navigate('/dashboard');
    } catch (error) {
      setStatus('Loan Repayment Failed');
      console.error(error);
    }
  };
  ...
  return (
    <div style={pageStyle}>
      ...
      <button onClick={handleRepayLoan} style={buttonStyle}>Repay Loan</button>
      <p>{status}</p>
    </div>
  );
};
